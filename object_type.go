...
func (t *ObjectType) setField(n *node, fieldName string, fieldValue any) {
	...
	if f.IsValid() && f.CanSet() {
		...
		} else if (fieldType.Kind() == reflect.Int || fieldType.Kind() == reflect.Int64) && fieldValueType != nil && fieldValueType.Kind() == reflect.Float32 {
			// Allow float32 to int assignments with truncation
			f.Set(reflect.ValueOf(int(fieldValue.(float32))))

		} else if (fieldType.Kind() == reflect.Int || fieldType.Kind() == reflect.Int64) && fieldValueType != nil && fieldValueType.Kind() == reflect.Float64 {
			// Allow float32 to int assignments with truncation
